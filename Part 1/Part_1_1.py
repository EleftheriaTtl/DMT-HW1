...
def MRR(Query_Total,SE_list):
    '''
    MRR(Q) = 1/|Q| sum(1/index_q(FirstRelevantResult)
    ...
    Returns
    -------
    The MRR(Mean Reciprocal Rank) of a Search Engine
...
def R_Precision_dist(Query_Total, SE_list):
    ...
    Returns
    -------
    The descriptive statistics corresponding to the 
    distribution of R-Precision given the various queries
    (mean, min, max, median, 1st quartile, 3rd quartile)
    '''
    R_P = {}
    for query_id in Query_Total['Query_id']:
...
def P_at_k_plot(Query_Total, SE_list, label):
    '''
    Plot Precision@k for diferent k in [1,3,5,10]
    '''
    p_a_k = {}
    ks = [1, 3, 5, 10]
    for k in ks:
...
def nDCG(query_id, k, Query_Total, SE_list):
    ...
    Returns
    -------
    The nDCG@k score for the a query, given the retrieved documents
    of a specific Search Engine considering the k positions
...
def nDCG_plot(Query_Total, SE_list, label):
    '''
    Plot nDCG@k for diferent k in [1,3,5,10]
    '''
    ndcg = {}
    ks = [1, 3, 5, 10]
...
def full_analysis(prefix, dataset_name, use_title):
    ...
    Returns
    -------
    conf_df : The different configurations
    MRR_df : The MRR score for each of the confifurations
    top_5 : To top 5 ranking configurations, based on the MRR score
    R_prec_df : The R-Precision descriptive statistics for each configuration
    ...
    # GT is the dataframe with our relevant documents for each query ranked
    # By groupby we're making a list of the relevant documents
    # where thankfully the ranking is kept 
    ...
    # In Query_Total, we combine the queries with their relevant
    # documents, so as to have everything we need in one data sctructure
    # In this way we also keep the queries that we can actually 
    # evaluate the results of, since not all queries have their 
    # according relevant documents in GT_list
...
# Time DATASET
t_conf_df,t_MRR_df, conf_df, t_MRR_df, t_top_5, t_R_prec_df = full_analysis('time', dataset_name = 'Time_DATASET', use_title=False)
